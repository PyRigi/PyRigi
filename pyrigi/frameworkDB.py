...
def Complete(n: int, dim: int = 2) -> Framework:
    ...
    * If `n-1<=dim`, then the complete graph is realized on the vertices of the
    `dim`-dimensional simplex. The complete graph is either given by the 1-skeleton
    of the simplex itself when `dim+1==n` or else as the 1-skeleton of a facet.
    * For `dim=1`, the complete graph is injectively realized on a the 1-dimensional
...
def Path(n: int, dim: int = 2) -> Framework:
    ...
    * For `dim=1`, the path is injectively realized on a the 1-dimensional linear space
